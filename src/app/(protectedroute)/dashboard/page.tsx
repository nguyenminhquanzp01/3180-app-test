...
const Dashboard = () => {
  ...
  const { data: apartmentsWithUnpaidFees = [] } =
  ...
  const residents =
    occupiedApartments?.apartments.flatMap(
          ...
          <div className="">
            <Tabs>
              <Tabs.TabPane
                tab={
                  <div className="flex items-center">
                    <DollarSign className="mr-2" /> Tổng quan{" "}
                  </div>
                }
                key="tab 1"
              >
                <div>
                  <Card className="mb-10 w-80 rounded-lg border border-gray-200 bg-white shadow-md hover:shadow-lg">
                    ...
                    <CardContent>
                      {isLoadingTotalContributionData ? (
                        ...
                        <div className="text-2xl font-bold text-gray-800">
                          ...
                          <p className="text-xs font-normal text-gray-500">
                            Tổng phí đóng góp:{" "}
                            {allTimeTotal.toLocaleString("vi-VN")}₫
                          </p>
                        </div>
                      )}
                    </CardContent>
                ...
                <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-7">
                  <Card className="col-span-4 rounded-lg border border-gray-200 bg-white shadow-md hover:shadow-lg">
                    <CardHeader>
                      <CardTitle className="text-blue-600">
                        Thanh toán gần đây
                      </CardTitle>
                      <CardDescription>
                        Chung cư BlueMoon có{" "}
                        {recentPayments ? recentPayments.length : [].length}{" "}
                        thanh toán mới
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      {isLoadingRecentPayments ? (
                        ...
                        <RecentPayments
                          payments={recentPayments ? recentPayments : []}
                        />
                      )}
                    </CardContent>
                  </Card>

                  <Card className="col-span-3 rounded-lg border border-gray-200 bg-white shadow-md hover:shadow-lg">
                    <CardHeader>
              ...
              <Tabs.TabPane
                tab={
                  <div className="flex items-center">
                    <BookX className="mr-2" /> Phí chưa thu
                  </div>
                }
                key="tab 2"
              >
                <div>
                  <Card className="w-80 rounded-lg border border-gray-200 bg-white shadow-md hover:shadow-lg">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-sm font-medium text-blue-500">
                        Phí chưa thu
                      </CardTitle>
                  ...
                  <Card className="mt-10">
                    <UnFeeList
                      apartmentList={apartmentsWithUnpaidFees.map(
                        (apartment) => ({
                          ...apartment,
                          unpaidFees: apartment.unpaidFees || [],
                        }),
              ...
              <Tabs.TabPane
                tab={
                  <div className="flex items-center">
                    <User className="mr-2" /> Cư dân Bluemoon
                  </div>
                }
                key="tab 3"
              >
              ...
              >
                <div>
                  ...
                  <Card className="mt-10">
                    <ApartmentList apartmentList={occupiedApartments?.apartments ?? []} />
                  </Card>
                </div>
              </Tabs.TabPane>
            </Tabs>
