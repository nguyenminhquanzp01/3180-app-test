...
export const feeRouter = createTRPCRouter({
  ...
  getAll: publicProcedure.query(async ({ ctx }) => {
    const fee = await ctx.db.fee.findMany({
      orderBy: {
        createAt: "desc",
      },
    });
  ...
  getTotalContributionFee: publicProcedure.query(async ({ ctx }) => {
    ...
    const currentMonthFees = await ctx.db.fee.findMany({
      where: {
        isPaid: true,
        dueDate: {
    ...
    const lastMonthFees = await ctx.db.fee.findMany({
      where: {
        isPaid: true,
        dueDate: {
          gte: lastMonthStart,
    ...
    const allTimeFees = await ctx.db.fee.findMany({
      where: {
        isPaid: true,
      },
      select: {
  ...
  getTotalUnpaidFees: publicProcedure.query(async ({ ctx }) => {
    const now = new Date();
    const unpaidFees = await ctx.db.fee.findMany({
      where: {
        isPaid: false,
        dueDate: {
          lt: now,
  ...
  getRecentPayments: publicProcedure.query(async ({ ctx }) => {
    const recentPayments = await ctx.db.fee.findMany({
      where: {
        isPaid: true,
      },
      orderBy: {
