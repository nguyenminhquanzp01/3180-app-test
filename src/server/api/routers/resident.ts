...
export const residentRouter = createTRPCRouter({
  ...
  getById: publicProcedure.input(z.string()).query(async ({ctx, input}) => {
    return ctx.db.resident.findUnique({
      where: {id: input},
      include: {
        address: {
          include: {
  ...
  update: publicProcedure
    ...
    .mutation(async ({ctx, input}) => {
      ...
      if (isStayingChangedToTrue) {
        await ctx.db.dateRange.deleteMany({
          where: {addressId: updatedAddress.id},
        });
      } else if (address.startDate && address.endDate) {
        const existingDateRange = await ctx.db.dateRange.findFirst({
          where: {addressId: updatedAddress.id},
        });

        if (existingDateRange) {
          await ctx.db.dateRange.update({
  ...
  delete: publicProcedure.input(z.string()).mutation(async ({ctx, input}) => {
    return ctx.db.resident.delete({
      where: {id: input},
    });
  }),

  getCount: publicProcedure.query(async ({ctx}) => {
    return ctx.db.resident.count();
  }),
  getOccupiedApartments: publicProcedure.query(async ({ ctx }) => {
    const occupiedApartments = await ctx.db.apartment.findMany({
      where: {
        addresses: {
          some: {
            isStaying: true,
